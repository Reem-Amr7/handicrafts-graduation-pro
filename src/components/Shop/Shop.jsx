...
const Shop = () => {
  ...
  const [localFavorites, setLocalFavorites] = useState({}); // حالة محلية للمفضلة
  ...
  const toggleFavorite = async (productId) => {
    ...
    if (!userId || !productId || !token) {
      ...
      throw new Error("بيانات غير مكتملة"); // رمي خطأ للتعامل معه في الدالة التي تستدعي
    }

    try {
      // نتحقق هل المنتج مفضل أصلًا
      ...
      throw err; // رمي الخطأ للتعامل معه في الدالة التي تستدعي
    }
  };
  ...
  const handleFavoriteClick = async (productId) => {
    ...
    try {
      ...
      // في حالة خطأ، نرجع الحالة السابقة
