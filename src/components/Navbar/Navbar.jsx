...
export default function Navbar() {
  ...
  // تحميل معلومات المستخدم عند تغيير التوكن
  useEffect(() => {
    if (token) {
      try {
        const decoded = jwtDecode(token);
        setUserId(decoded.sub);
        setUserName(decoded.name || decoded.unique_name);
        
        const roles = decoded["http://schemas.microsoft.com/ws/2008/06/identity/claims/role"];
        const adminStatus = Array.isArray(roles) ? roles.includes("Admin") : roles === "Admin";
        setIsAdmin(adminStatus);
        
        localStorage.setItem("userId", decoded.sub);
        localStorage.setItem("userName", decoded.name || decoded.unique_name);
      } catch (error) {
  ...
  return (
    <header className="bg-gradient-to-r from-[#8B4513] to-[#5D4037] text-white shadow-md h-24 fixed top-0 w-full z-30">
      ...
      <div className="container mx-auto flex justify-between items-center py-3 px-8 relative">
        ...
        {token && (
          <nav className="hidden md:block">
            <ul className="flex text-lg flex-row-reverse gap-6">
              {isAdmin && (
                <li><Link to="/admin" className={`${styles.navLink} ${location.pathname === '/admin' ? styles.activeLink : ''}`}> الادمن</Link></li>
              )}
        ...
        <div className="flex items-center gap-3 flex-row-reverse">
          {token ? (
            <>
              ...
              <div onClick={() => navigate('/cart')} className="w-10 h-10 flex items-center justify-center rounded-full bg-white/10 hover:bg-white/20 text-xl text-white cursor-pointer">
            ...
            <div className="flex gap-3">
              <Link to="/login" className="bg-[#2E230D] text-white px-4 py-2 rounded-lg hover:bg-[#A68B55]">تسجيل الدخول</Link>
              <Link to="/register" className="bg-[#A68B55] text-white px-4 py-2 rounded-lg hover:bg-[#2E230D]">إنشاء حساب</Link>
            </div>
          )}
          ...
          <div className="relative" ref={searchRef}>
            <div className="flex items-center">
              ...
              />
              {!showSearch && (
                <FaSearch className="absolute left-4 top-4 text-black cursor-pointer" onClick={handleShowSearch} />
              )}
            </div>
        ...
        <FaBars className="md:hidden text-2xl cursor-pointer" onClick={() => setIsMenuOpen((prev) => !prev)} />
      </div>
      ...
      {isMenuOpen && token && (
        <nav className="absolute top-24 right-0 w-full bg-[#5D4037] md:hidden z-40">
          <ul className="flex flex-col text-xl items-end p-4 gap-3">
            {isAdmin && (
              <li><Link to="/admin" onClick={() => setIsMenuOpen(false)} className={`${styles.navLink} ${location.pathname === '/admin' ? styles.activeLink : ''}`}>لوحة التحكم</Link></li>
            )}
            <li><Link to="/foryou" onClick={() => setIsMenuOpen(false)} className={`${styles.navLink} ${location.pathname === '/foryou' ? styles.activeLink : ''}`}>مقترح لك</Link></li>
